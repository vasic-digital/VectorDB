...
echo "=== VectorDB Stress Sustained-Load Challenge ==="
echo "  url=$HEALTH_URL dur=${DURATION}s rps=${RPS}"

if [[ -z "$HEALTH_URL" ]]; then
    echo "[1/6] SKIP: VECTORDB_HEALTH_URL unset — SKIP-OK: #env-no-target"
    echo "=== VectorDB Stress Challenge: PASSED (SKIP-OK) ==="
    exit 0
fi
...
echo
echo "=== VectorDB Stress Challenge: PASSED ==="
